/**
 * This header is generated by class-dump-z 0.2a.
 * class-dump-z is Copyright (C) 2009 by KennyTM~, licensed under GPLv3.
 *
 * Source: /System/Library/PrivateFrameworks/iTunesStoreUI.framework/iTunesStoreUI
 */

#import "SUInputSource.h"

@class NSURL;

@interface SUFileInputSource : SUInputSource {
	long long _expectedLength;
	int _fd;
	NSURL* _url;
}
@property(readonly, assign, nonatomic) NSURL* fileURL;	// @synthesize=_url
@property(readonly, assign) long long expectedLength;	// converted property
// declared property getter: -(id)fileURL;
-(void)_close;
-(int)read:(char*)read maxLength:(unsigned)length error:(id*)error;
-(BOOL)open:(id*)open;
-(BOOL)hasBytesAvailable;
// converted property getter: -(long long)expectedLength;
-(void)close;
-(void)dealloc;
-(id)initWithFileURL:(id)fileURL;
-(id)init;
@end

